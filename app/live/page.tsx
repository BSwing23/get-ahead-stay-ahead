...
export default function LivePage() {
  ...
  const convention   = useMatchStore(s => (s as any).convention ?? 'International') as 'International'|'American'
  ...
  // Zone label respects convention
  const zoneLabel = convention === 'International' ? `Z${currentRot}` : `${currentRot}`
  ...
  // Button ordering follows the scoreboard side
  const leftBtn = {
    label: leftIsMy ? `${myName} +1` : `${oppName} +1`,
    onClick: () => commitRally(leftIsMy ? 'my' : 'opp'),
    style: {
      background: leftIsMy ? '#0d6efd' : '#dc3545',
      color: '#fff'
    } as React.CSSProperties
  }
  const rightBtn = {
    label: leftIsMy ? `${oppName} +1` : `${myName} +1`,
    onClick: () => commitRally(leftIsMy ? 'opp' : 'my'),
    style: {
      background: leftIsMy ? '#dc3545' : '#0d6efd',
      color: '#fff'
    } as React.CSSProperties
  }
    ...
    <main style={{padding:'20px', maxWidth:1100, margin:'0 auto'}}>
      ...
      >
        ...
        <div style={{textAlign:'center'}}>
          ...
          <div style={{fontSize:12, opacity:.7, marginTop:8}}>Zone: {zoneLabel}</div>
        </div>
      ...
      {/* Buttons (follow left/right side) */}
      ...
      <section style={{marginTop:20}}>
        ...
        <div style={{marginTop:8, fontSize:11, opacity:.55}}>
          Neutral-rally model (no sideout bias). We can later upgrade to a PS/SO-aware win model.
        </div>
      </section>
        ...
        {recent.length === 0 ? (
          ...
          <ul style={{listStyle:'none', padding:0, margin:0, display:'grid', gap:8}}>
            {recent.map(r => {
              ...
              const zoneTxt = convention === 'International' ? `Z${r.myRotationAtStart}` : `${r.myRotationAtStart}`
